"""Phase 5: Interestingness Framework.

Defines 9 archetypes with explicit, transparent scoring rules.
...
print_top_papers(df, "score_top_overall", "TOP OVERALL")
...
hidden_gem_df = df.filter(pl.col("status").str.contains("Poster"))
...
print_top_papers(df, "score_controversial", "CONTROVERSIAL", 15)

# ══════════════════════════════════════════════════════════════════════════
# 4. HIGH ENGAGEMENT
# Lots of discussion, long reviews, many questions (area-relative)
# ══════════════════════════════════════════════════════════════════════════
...
print_top_papers(df, "score_high_engagement", "HIGH ENGAGEMENT", 15)

# ══════════════════════════════════════════════════════════════════════════
# 5. SEMANTICALLY NOVEL (embedding-dependent)
# ══════════════════════════════════════════════════════════════════════════
...
if has_embeddings:
    ...
    print_top_papers(df, "score_semantic_novel", "SEMANTICALLY NOVEL", 15)
else:
    print("\n⚠ Skipping SEMANTICALLY NOVEL (no embeddings)")

# ══════════════════════════════════════════════════════════════════════════
# 6. BRIDGE PAPERS (embedding-dependent)
# ══════════════════════════════════════════════════════════════════════════
    ...
    if exemplars:
        ...
        for row in exemplar_df.iter_rows(named=True):
            print(
                ...
                f"rating={row['rating_mean']:.1f}  {row['title'][:65]}"
            )
else:
    print("\n⚠ Skipping CLUSTER EXEMPLARS (no clusters)")
...
for area in areas:
    ...
    for row in top.iter_rows(named=True):
        print(
            f"    rating={row['rating_mean']:.1f} score={row['score_area_leader']:.2f}  "
            f"{row['title'][:65]}"
        )

# ══════════════════════════════════════════════════════════════════════════
# 9. CONSENSUS STANDOUTS
# Unanimously praised: high rating + low std + high confidence
# ══════════════════════════════════════════════════════════════════════════
...
print_top_papers(df, "score_consensus", "CONSENSUS STANDOUTS", 15)

# ── Save ─────────────────────────────────────────────────────────────────
score_cols = [c for c in df.columns if c.startswith("score_")]
print(f"\nArchetype scores computed: {score_cols}")

df.write_parquet("iclr_2026_scored.parquet")
print(f"Saved to iclr_2026_scored.parquet ({df.shape[0]} rows × {df.shape[1]} columns)")
