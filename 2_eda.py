...
fig = px.bar(
    ...
    x="primary_area", y="len", color="status",
    barmode="stack",
...
fig.update_layout(title="Score Distributions (paper-level means)", height=350, width=1400)
fig.write_html(FIGURES / "score_distributions.html")

# ── 4. Review verbosity ─────────────────────────────────────────────────
wc_cols = ["wc_review_mean", "wc_strengths_mean", "wc_weaknesses_mean",
           "wc_questions_mean", "wc_summary_mean"]
...
all_numeric = score_cols + [
    "rating_std", "n_reviewers", "n_replies", "n_authors",
    "total_review_wc", "wc_review_mean", "wc_strengths_mean",
    "wc_weaknesses_mean", "wc_questions_mean",
    "strengths_weaknesses_ratio", "questions_review_ratio",
...
fig = px.histogram(
    df.to_pandas(), x="rating_std",
    nbins=30, title="Rating Standard Deviation Distribution",
)
fig.write_html(FIGURES / "rating_std_distribution.html")

# Disagreement vs rating level
fig = px.scatter(
    df.to_pandas(), x="rating_mean", y="rating_std",
    opacity=0.3, title="Rating Mean vs Std (Disagreement)",
    trendline="lowess",
)
...
fig = px.scatter(
    df.to_pandas(), x="rating_std", y="wc_review_mean",
    opacity=0.3, title="Disagreement vs Review Length",
    trendline="lowess",
)
...
fig = px.histogram(
    df.to_pandas(), x="corr_rating_confidence",
    nbins=30, title="Per-Paper Rating-Confidence Correlation",
)
fig.write_html(FIGURES / "corr_rating_confidence_dist.html")

# Do confident reviewers rate higher or lower?
fig = px.scatter(
    df.to_pandas(), x="confidence_mean", y="rating_mean",
    opacity=0.3, title="Confidence Mean vs Rating Mean",
    trendline="ols",
)
...
fig = px.scatter(
    df.to_pandas(), x="rating_mean", y="wc_review_mean",
    opacity=0.3, title="Rating vs Mean Review Word Count",
    trendline="lowess",
)
...
fig = px.scatter(
    df.to_pandas(), x="rating_mean", y="n_replies",
    opacity=0.3, title="Rating vs Reply Count",
    trendline="lowess",
)
...
fig = px.scatter(
    df.to_pandas(), x="rating_mean", y="strengths_weaknesses_ratio",
    opacity=0.3, title="Rating vs Strengths/Weaknesses Word Count Ratio",
    trendline="lowess",
)
...
fig = px.scatter(
    ...
    x="mean_rating", y="mean_soundness",
    size="count", hover_name="primary_area",
...
for col in score_cols + ["rating_std", "wc_review_mean", "n_replies"]:
    ...
    # Cohen's d
    pooled_std = np.sqrt(((len(o) - 1) * o.std(ddof=1)**2 + (len(p) - 1) * p.std(ddof=1)**2)
                          / (len(o) + len(p) - 2))
    ...
    print(f"  {col:30s}  Cohen's d={d:+.3f}  rank-biserial={rank_biserial:+.3f}  p={p_val:.2e}")
